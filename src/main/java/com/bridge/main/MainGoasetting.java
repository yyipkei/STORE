...
public class MainGoasetting implements Job {
	
	public void execute(JobExecutionContext arg0) throws JobExecutionException {
		bridgeStart();
	}

	private static final Logger logger = Logger.getLogger(MainGoasetting.class);

	public static void bridgeStart() {
		/*
		 * try {
		 * 
		 * runStoredProcedure("Oracle"); selectRecordsFromTable("Oracle"); for
		 * (Dataupdatelog dataupdatelog : dataupdatelogs)
	...
	public static void processLog(String database) {
		for (Dataupdatelog d : MainGoasetting.dataupdatelogs) {
			RouteGoasetting.Routeing(d.getDatalogid(), d.getEntityname(),
	...
	private static void selectRecordsFromTable(String database)
		...
		String selectSQL = "SELECT DATA_UPDATE_LOG_ID ,ENTITY_NAME, ENTITY_KEY,ENTITY_UPD_DT,"
				+ " LOG_DT,BATCH_NO,IS_COMP,REMARK FROM DATA_UPDATE_LOG_POS where IS_COMP ='P' and REMARK ='Goasetting'";
	...
	private static void runlogStoredProcedure(String database)
		...
		PreparedStatement preparedStatement = null;
		logger.info("Starting ");
		String spSQL = "{call usp_data_update_log_goaset}";
		try {
