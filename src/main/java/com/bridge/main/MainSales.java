...
public class MainSales implements Job {
    ...
    public static void processLog(String database) {
        ...
        for (Dataupdatelog d : MainSales.dataupdatelogs) {
           ...
           */
        int size = dataupdatelogs.size();
        for (int j = 0; j < size; j++) {
            RouteSales.Routeing(dataupdatelogs.get(j).getDatalogid(), dataupdatelogs.get(j).getEntityname(), dataupdatelogs.get(j).getEntitykey(), database);
        }
...
/*
        ...
        for (Dataupdatelog d : MainSales.dataupdatelogs) {
            elems.add(Integer.valueOf(d.getDatalogid()));
        }
...
*/
    ...
    public static void processLogmssql(String database) {
        int size = dataupdatelogs.size();
        for (int j = 0; j < size; j++) {
            if (!Objects.equals(dataupdatelogs.get(j).getEntityname(), "sahdr") && !Objects.equals(dataupdatelogs.get(j).getEntityname(), "sadet")
                    && !Objects.equals(dataupdatelogs.get(j).getEntityname(), "satender")) {
                RouteSales.Routeing(dataupdatelogs.get(j).getDatalogid(), dataupdatelogs.get(j).getEntityname(), dataupdatelogs.get(j).getEntitykey(), database);
            }
        }

        size = dataupdatelogs.size();
        for (int j = 0; j < size; j++) {
            if (Objects.equals(dataupdatelogs.get(j).getEntityname(), "sahdr")) {
                RouteSales.Routeing(dataupdatelogs.get(j).getDatalogid(), dataupdatelogs.get(j).getEntityname(), dataupdatelogs.get(j).getEntitykey(), database);
            }
        }
        ...
        size = dataupdatelogs.size();
        for (int j = 0; j < size; j++) {
            if (Objects.equals(dataupdatelogs.get(j).getEntityname(), "satender")) {
                RouteSales.Routeing(dataupdatelogs.get(j).getDatalogid(), dataupdatelogs.get(j).getEntityname(), dataupdatelogs.get(j).getEntitykey(), database);
            }
        }
        ...
        size = dataupdatelogs.size();
        for (int j = 0; j < size; j++) {
            if (Objects.equals(dataupdatelogs.get(j).getEntityname(), "sadet")) {
                RouteSales.Routeing(dataupdatelogs.get(j).getDatalogid(), dataupdatelogs.get(j).getEntityname(), dataupdatelogs.get(j).getEntitykey(), database);
            }
        }
...
/*
        ...
        System.out.println("222Using [int size = list.size(); int j = 0; j < size ; j++] :: " + (endTime - startTime) + " ms");
*/
    }
