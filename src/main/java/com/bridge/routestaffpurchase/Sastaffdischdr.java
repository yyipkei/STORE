...
import com.bridge.insertstaffpurchase.InsertSastaffdischdr;
import com.bridge.main.HikariMssql;
import com.bridge.main.HikariQracleFrom;
import com.bridge.result.Logupdateresult;

public class Sastaffdischdr {

	private static final Logger logger = Logger.getLogger(Sastaffdischdr.class);

	public static void routeSastaffdischdr(String dataupdatelog,
		...
		selectSQL = "SELECT COMPANY_CDE,BATCH_NO,BATCH_DESC,VER_NO,CURR_VER,"
				+ "STATUS,EFF_DT_FR,EFF_DT_TO,PREPARE_BY,CFM_DT,CFM_BY,"
				+ "APP_DT,APP_BY,LAST_UPD_DT,LAST_UPD_USR,LAST_UPD_VER,"
				+ "FM_COMPANY,FM_BATCH,FM_VER,ENTITY_KEY "
				+ "FROM dbo.SA_STAFF_DISC_HDR_pos " + "where entity_key ='" + entitykey
				+ "'"
		...
		try {
			...
			while (rs.next()) {
				...
				boolean Chkresult = InsertSastaffdischdr
						.Sastaffdischdrchkexists(entitykey, database);

				if (!Chkresult) {

					// logger.info("insert");

					boolean Insertresult = InsertSastaffdischdr
							.Sastaffdischdrinsert(rscompanycde, rsbatchno,
					...
					if (Insertresult) {
						logger.info("Sastaffdischdr: 1 row has been inserted. Key:"
								+ entitykey);
					} else {
					...
					boolean Insertresult = InsertSastaffdischdr
							.Sastaffdischdrupdate(rscompanycde, rsbatchno,
									rsbatchdesc, rsverno, rscurrver, rsstatus,
									rseffdtfr, rseffdtto, rsprepareby, rscfmdt,
									rscfmby, rsappdt, rsappby, rslastupddt,
									rslastupdusr, rslastupdver, rsfmcompany,
					...
					if (Insertresult) {
						logger.info("Sastaffdischdr: 1 row has been updated. Key:"
								+ entitykey);
					} else {
